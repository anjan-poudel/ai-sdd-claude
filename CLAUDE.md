...
**Confidence overlay** — `ConfidenceOverlay` gates task completion when `score < threshold` (default `0.7`). Below threshold → `NEEDS_REWORK`. A second optional `low_confidence_threshold` (disabled until explicitly set) triggers a regeneration + escalation chain: regen retries (with per-attempt `top_p`/`temperature` nudges via `regen_sampling_schedule`) → paired challenger (once, if paired enabled) → HIL (manual resolution → rework, or reject → FAILED). Regen retries do not consume `max_rework_iterations` budget. Default sampling schedule: `[{top_p:0.9,temperature:0.2}, {top_p:0.8,temperature:0.4}, {top_p:0.7,temperature:0.6}]`.

**Traceability overlay** — LLM-judge post-task gate that reads the requirements lock file and verifies task outputs stay within requirements scope. Enabled by default (`ENGINE_TASK_DEFAULTS`); gracefully skips when no lock file exists. Runs on `design` + `implement` phases. Returns `NEEDS_REWORK` if the judge finds out-of-scope elements. `evaluator_agent` auto-resolves to `reviewer` at runtime; explicit override via `overlays.traceability.evaluator_agent` (must differ from task agent, validated at load time). If auto-resolved agent equals the task agent, the overlay skips to avoid self-evaluation. Lock file path: `specs/define-requirements.lock.yaml` (configurable via `lock_file`).
...
**Workflow defaults + task library** — `workflow-loader.ts` applies a 4-layer merge per task: `ENGINE_TASK_DEFAULTS` → workflow `defaults:` block → task library template (`use:`) → task inline. Engine built-ins: `hil.enabled=true`, `policy_gate.risk_tier=T1`, `traceability.enabled=true`, `max_rework_iterations=3`. Overlay keys merge individually (not whole-object replace). `{{task_id}}` in library output paths is substituted at load time. Templates supply a `phase` field (`requirements`, `design`, `planning`, `implement`, `review`, `sign-off`) used by remote overlays for phase-based filtering.
