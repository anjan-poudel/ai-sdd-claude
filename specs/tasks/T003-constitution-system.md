...
Feature: Constitution resolver
  ...
  Scenario: Feature constitution from specs/ is included in merge
    Given a project with constitution.md at root
    And a feature constitution at specs/auth/constitution.md
    When a task runs
    Then both constitutions are merged (root first, then feature)
    And the resolved content includes content from both files

  Scenario: Feature constitutions merge in alphabetical order
    Given feature constitutions at specs/a-auth/ and specs/b-payments/
    When the resolver runs
    Then specs/a-auth/constitution.md appears before specs/b-payments/constitution.md

  Scenario: Empty specs directory does not break resolution
    Given a project with constitution.md at root and an empty specs/ directory
    When the resolver runs
...
3. `.ai-sdd/constitution.md` (engine manifest)
4. `CLAUDE.md` (Claude Code convention)
5. Feature constitutions: `specs/*/constitution.md` (alphabetical by directory name)
6. Submodule constitutions: `<submodule>/constitution.md` (one level deep)

Feature constitutions live in `specs/<feature-slug>/constitution.md` — NOT inside `.ai-sdd/`.
The `.ai-sdd/` directory is reserved for framework runtime files and must remain static across
all projects. Feature-specific artifacts (constitutions, init reports, design docs) belong in
`specs/<feature-slug>/`.
...
- **Feature constitutions**: additive — merged after root, before submodules. They extend the
  project constitution with feature-scoped constraints, not override it.
- **AUTO-GENERATED section** (`## Workflow Artifacts`): engine-written post-task; lives in project root only — never merged into sub-module or feature constitutions.
